...
interface WorkspaceState {
  ...
  /**
   * Whether the server can actually create a repository there, and why not.
   *
   * Carried beside `initRoot` because the path alone is not a diagnosis: it is a path *inside a
   * container*, which the person reading it cannot cd to, chmod, or change from a browser. The
   * reason and the hint are the only parts they can act on.
   */
  initDiagnosis: RootDiagnosis | undefined;
...
export function WorkspaceProvider({ children }: { children: ReactNode }): JSX.Element {
  ...
  const [initDiagnosis, setInitDiagnosis] = useState<RootDiagnosis | undefined>();
    ...
    api
      ...
      .catch((err: unknown) => {
        ...
        if (err instanceof ApiError && err.needsInit) {
          ...
          setInitDiagnosis(err.rootDiagnosis);
