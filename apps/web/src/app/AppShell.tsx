...
export function AppShell({
  ...
  */
  ...
  /*
    Below 900px the labels do not fit, so the nav is collapsed whatever the preference says.

    This used to be done in CSS, with `@media (max-width: 900px)` restating the
    `.nav--collapsed` rules. It restated three of the eight and drifted: the dirty marker,
    the twisties, the depth indents and the collapse button's own label all kept their
    expanded styling inside a 48px rail, and that label clipped to 35px. Deriving the class
    here means there is one description of what collapsed looks like instead of two.

    The stored preference is deliberately not overwritten. Someone who expanded the nav on a
    wide screen should get it back when the window grows again, not have the width silently
    forgotten because they once opened a narrow one.
  */
  const navTooNarrow = useMediaQuery("(max-width: 900px)");
  ...
  return (
    <ShellActionsContext.Provider value={actions}>
      ...
      <div className="shell__nav">
        <Sidebar
          router={router}
          collapsed={navCollapsed || navTooNarrow}
          /*
            No toggle when the width forces the issue: a button that cannot change what you
            see is worse than an absent one.
          */
          onToggleCollapsed={
            navTooNarrow ? undefined : () => setNavCollapsed((current) => !current)
          }
        />
      </div>

      {/*
        Read-only is announced, not merely enforced.

        The server has always refused a viewer's writes, and the UI has always greyed out the
        controls that would be refused. Neither ever said *why*, so a viewer opened the app to a
        greyed-out interface with no explanation and could not tell "forbidden" from "broken".
        That matters more now than it looks: inviting a stakeholder as a viewer is the answer to
        sharing a model with people who will never install this, and their whole first impression
        was a dead screen.

        Stated once here rather than as a tooltip on each of the twenty-three components that
        take `canEdit`. Once the banner has said it, a tooltip per control is noise, and the
        version of this that lives in one place cannot drift out of step with itself.

        Not dismissible, because it does not become untrue.
      */}
      <main className={`shell__main${canEdit ? "" : " shell__main--banner"}`}>
        {canEdit ? null : (
          <div className="readonly" role="status">
            <Icon name="lock" size={12} />
            <span>
              <strong>Read only.</strong> You have {user?.role ?? "viewer"} access, so you can open
              everything here and change none of it. An administrator can give you editor access.
            </span>
          </div>
        )}
        {children}
