...
/**
 * Decode one path segment, treating a malformed escape as literal text.
 *
 * `decodeURIComponent` throws `URIError` on a lone `%` or a truncated escape like `%E0%A4%A`,
 * and this runs during render. Unguarded, one stray character in the address bar took the whole
 * app down to a blank screen, which is both the worst possible outcome and the exact opposite of
 * what the fallback below promises: an unrecognised URL is supposed to land on the overview.
 *
 * A mistyped address is a routing miss, not an exception. Handing back the raw segment lets it
 * fail the way every other unknown path already does.
 */
