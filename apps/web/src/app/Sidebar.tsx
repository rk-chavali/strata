...
export function Sidebar({
  ...
  collapsed: boolean;
  /** Absent when the viewport is too narrow for the expanded nav to be an option. */
    ...
    <nav className={`nav${collapsed ? " nav--collapsed" : ""}`} aria-label="Main">
      <div className="nav__scroll">
        ...
        */}
        ...
        {/*
          Never gated on a capability. The report is computed from the model alone, so it works on
          a workspace with no integrations, no credentials and no features enabled, and hiding it
          behind a flag would make the one page written for a compliance reviewer the one they
          could not reach.
        */}
        <NavItem
          router={router}
