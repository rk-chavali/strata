...
/** The ticket behind a change. Mirrors `TicketRef` in `apps/server/src/provenance.ts`. */
export interface TicketRef {
  key: string;
  url?: string;
  summary?: string;
  status?: string;
  /** Why summary and status are missing. Shown, so the panel never looks merely broken. */
  unavailable?: string;
}

/** Why an object exists. Mirrors `ObjectProvenance` in `apps/server/src/provenance.ts`. */
export interface ObjectProvenance {
  ...
  ticket?: TicketRef;
  /**
   * Set when a ticket source was actually consulted.
   *
   * Absent means Jira is not configured, so nothing looked. The panel needs the distinction:
   * "no ticket in this commit" and "no ticket system connected" call for opposite next steps.
   */
  ticketSource?: "jira";
  unavailable?: string;
}
...
/**
 * Whether a repository can be created where the deployment is pointed.
 *
 * Mirrors `RootDiagnosis` in `apps/server/src/workspaceroot.ts`. The path itself is a *container*
 * path, which the person reading it cannot cd to, chmod, or change from a browser, so the reason
 * and the hint are the parts they can actually act on.
 */
export interface RootDiagnosis {
  writable: boolean;
  reason?: string;
  /** A named, certain misconfiguration. Absent when the cause could be any of several things. */
  hint?: string;
}

// ---------------------------------------------------------------- governance

/** Mirrors the report in `apps/server/src/governance.ts`. */
export interface GovernanceReport {
  ...
  models: { model: string; classified: number; total: number; unrecognised: number }[];
  register: {
    model: string;
    object: string;
    field: string;
  ...
  /** Unclassified fields no pattern recognises, so a human has to decide. Not the same as unclassified. */
  backlog: { model: string; object: string; field: string; type: string }[];
  suggestions: number;
  truncated: boolean;
  generatedAt: string;
}
