...
describe("authentication", () => {
  it("builds a signed JWT with the fields Google requires", () => {
    const assertion = buildAssertion("svc@acme.iam.gserviceaccount.com", privateKey, "https://x/token", 1_000);
  ...
  it("accepts a private key with literal escaped newlines", () => {
    ...
    */
    ...
    expect(() => buildAssertion("svc@acme", escaped, "https://x/token")).not.toThrow();
  });
