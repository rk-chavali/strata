...
/**
 * One scope per API, rather than one scope for everything.
 *
 * This was a single `cloud-platform` constant shared by both call sites, which is every
 * Google API the key can reach, requested by something that reads schemas.
 *
 * Worth being precise about what that did and did not mean: for a service account the
 * effective permission is the scope intersected with the account's IAM roles, so a key
 * holding only `bigquery.metadataViewer` was never able to act outside BigQuery whatever
 * it asked for. The breadth was a missing second lock, not an open door. It is still worth
 * closing: the token minted here is a bearer credential, and if one leaks the scope is the
 * only thing still constraining it.
 *
 * The minimum IAM roles are documented beside these, because the roles are the lock that
 * actually binds and no scope choice substitutes for getting them right.
 */
export const GCP_SCOPES = {
  /*
    Not `bigquery.readonly`. Reading `INFORMATION_SCHEMA` means running a query, a query is
    a job, and creating a job is a write in IAM's eyes, which the read-only scope refuses.
    Least privilege here is `roles/bigquery.metadataViewer` plus `roles/bigquery.jobUser`:
    it can run the metadata queries and read no table data.
  */
  bigquery: "https://www.googleapis.com/auth/bigquery",

  /*
    Data Catalog v1 publishes no scope narrower than `cloud-platform`, so this one cannot be
    tightened from our side. It is requested only when reading policy-tag taxonomies, and
    `roles/datacatalog.viewer` alone keeps the account read-only in practice.
  */
  dataCatalog: "https://www.googleapis.com/auth/cloud-platform",
} as const;

export type GcpScope = (typeof GCP_SCOPES)[keyof typeof GCP_SCOPES];
...
export function buildAssertion(
  ...
  privateKey: string,
  audience: string,
  /* Required rather than defaulted: a forgotten argument should not silently ask for more. */
  scope: GcpScope,
...
/**
 * Resolve whatever the operator configured into a bearer token for one scope.
 *
 * A pre-supplied `kind: "token"` is returned as it stands. Its scope was fixed by whoever
 * minted it, so this cannot narrow it and does not pretend to.
 */
export async function accessToken(
  ...
  const response = await fetcher(uri, {
    ...
    body: new URLSearchParams({
      ...
      assertion: buildAssertion(auth.clientEmail, auth.privateKey, uri, scope),
    }),
...
export async function listTaxonomies(
  ...
  const token = await accessToken(options.auth, GCP_SCOPES.dataCatalog, fetcher);
...
export async function runQuery(
  ...
  const token = await accessToken(options.auth, GCP_SCOPES.bigquery, fetcher);
