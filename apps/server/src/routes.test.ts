...
function routesOf(app: Express): RouteInfo[] {
  ...
  /*
    Descends into mounted routers as well as reading the top level.

    The flat version read `_router.stack` and looked at `layer.route`, which is every route only
    while every route is registered directly on `app`. The moment any group moves behind
    `app.use(router)` its routes live one level down in `layer.handle.stack`, and a flat read
    walks straight past them: no error, no empty result, just an audit quietly covering less than
    it claims to. Both audits below are allow-list shaped, so a route they cannot see is a route
    that passes. That is the failure this recursion exists to prevent.
  */
  const walk = (stack: unknown[]): void => {
    for (const layer of stack) {
      const entry = layer as {
        route?: { path: unknown; stack: { handle: GuardHandler }[]; methods: Record<string, boolean> };
        handle?: { stack?: unknown[] };
      };

      if (entry.route && typeof entry.route.path === "string") {
        const guards = entry.route.stack.map((item) => item.handle);
      ...
      // Plain middleware is a function with no `stack`, so this only follows routers.
      if (Array.isArray(entry.handle?.stack)) walk(entry.handle.stack);
    }
  };
...
describe("route audit", () => {
  ...
  it("finds a substantial number of routes, so a broken walk cannot pass vacuously", async () => {
    /*
      ...
      The floor sits just under the real count rather than comfortably below it. At 50 it was
      loose enough that half the routes could disappear from the walk and still pass, which is
      the exact shape of the failure that moving routes behind a mounted router would cause.
      Close to the real number, a whole group going missing is a red test.
    */
    const { app } = await boot();
    expect(routesOf(app).length).toBeGreaterThan(95);
  });
});
...
describe("creating a workspace at an unusable path", () => {
  /**
   * The failure an operator actually hits, and the one that told them nothing.
   *
   * Git Bash on Windows rewrites any argument shaped like a Unix path, so
   * `-e STRATA_WORKSPACE=/app/examples/quickstart` reaches the container as
   * `C:/Program Files/Git/app/examples/quickstart`. The workspace then does not load, the UI
   * offers to create one, `mkdir` fails with EACCES on a directory the container does not own,
   * and the operator sees "Something went wrong on the server." The diagnosis was only ever in a
   * container log they had no reason to open.
   *
   * Reproduced here with a parent that is a *file* rather than a permissions trick, because
   * chmod is not portable to the Windows leg of this matrix and the property under test is not
   * "EACCES specifically", it is "any filesystem error names the path and the reason".
   */
  it("names the path and the reason instead of answering with a blank 500", async () => {
    const base = await mkdtemp(join(tmpdir(), "strata-badroot-"));
    tempRoots.push(base);

    // A regular file where the workspace's parent directory should be. `mkdir` under it cannot
    // succeed on any platform.
    const blocker = join(base, "notadir");
    await writeFile(blocker, "this is a file, not a directory", "utf8");
    const unusable = join(blocker, "workspace");

    const booted = await boot({ STRATA_WORKSPACE: unusable });
    await createUser(booted, "root", "correct-horse-battery-staple-9", "admin");
    const cookie = await signIn(booted, "root", "correct-horse-battery-staple-9");

    const response = await fetch(`${booted.url}/api/workspace/init`, {
      method: "POST",
      headers: { "content-type": "application/json", cookie },
      body: JSON.stringify({ name: "warehouse" }),
    });

    const body = (await response.json()) as { error?: string; root?: string; code?: string };

    // 422, not 500: the request is fine, the deployment's path is not.
    expect(response.status).toBe(422);
    /*
      The wording comes from `diagnoseRoot`, shared with the preflight on `/api/workspace`, so
      the two cannot describe the same deployment differently. What is asserted here is the
      property rather than the sentence: it names a real path, carries a filesystem code, and is
      not the generic handler message.
    */
    expect(body.error).toMatch(/Cannot (write to|create a repository at)/);
    expect(body.error).toContain(blocker);
    // The path is echoed back. An operator seeing a mangled one knows their shell did it.
    expect(body.root).toBe(unusable);
    expect(body.code).toBeTruthy();
    expect(body.error).not.toContain("Something went wrong");
  });

  it("tells the setup screen it cannot write there, before offering the button", async () => {
    /*
      The actual fix. `/api/workspace` used to answer `needsInit: true` for any directory without
      a config file, writable or not, so the UI always offered "Create repository" and the
      operator found out by pressing it. Deciding it here turns a dead control into a sentence
      they can act on, which matters because the path is inside a container and they cannot
      change it from a browser anyway.
    */
    const base = await mkdtemp(join(tmpdir(), "strata-badroot2-"));
    tempRoots.push(base);

    const blocker = join(base, "notadir");
    await writeFile(blocker, "a file, not a directory", "utf8");

    const booted = await boot({ STRATA_WORKSPACE: join(blocker, "workspace") });
    await createUser(booted, "root", "correct-horse-battery-staple-9", "admin");
    const cookie = await signIn(booted, "root", "correct-horse-battery-staple-9");

    const response = await fetch(`${booted.url}/api/workspace`, { headers: { cookie } });
    ...
    // The half that is new, and the half the UI keys off.
    expect(body.writable).toBe(false);
    expect(body.reason).toBeTruthy();
  });

  it("reports a writable root as writable, so an ordinary first run is unaffected", async () => {
    const base = await mkdtemp(join(tmpdir(), "strata-goodroot-"));
    tempRoots.push(base);

    const booted = await boot({ STRATA_WORKSPACE: join(base, "fresh") });
    await createUser(booted, "root", "correct-horse-battery-staple-9", "admin");
    const cookie = await signIn(booted, "root", "correct-horse-battery-staple-9");

    const response = await fetch(`${booted.url}/api/workspace`, { headers: { cookie } });
    const body = (await response.json()) as { needsInit?: boolean; writable?: boolean };
