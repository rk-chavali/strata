...
function summaryFor(overrides: Partial<ChangeSummary> = {}): ChangeSummary {
  return {
    ...
    governance: [{ model: "retail", classified: 18, total: 24, unrecognised: 4 }],
...
describe("markdownReview", () => {
  ...
  it("renders the diagram in a mermaid fence, which is what GitHub actually draws", () => {
    const body = markdownReview(summaryFor());
    expect(body).toContain("```mermaid");
    expect(body).toContain("erDiagram");
  });

  it("puts the diagram above the consequences, because shape is read before impact", () => {
    const body = markdownReview(summaryFor());
    expect(body.indexOf("```mermaid")).toBeLessThan(body.indexOf("### Downstream impact"));
  });

  it("omits the diagram section entirely when the change touched no tables", () => {
    // An empty ```mermaid fence renders as a parse error box, which is worse than no diagram.
    const body = markdownReview(summaryFor({ erd: undefined }));
    expect(body).not.toContain("```mermaid");
  });

  it("reports classification coverage and the count that needs a human", () => {
    const body = markdownReview(summaryFor());
    expect(body).toContain("### Governance");
    expect(body).toContain("18 / 24 (75%)");
  });

  it("omits governance rather than printing 0% for a model with no columns", () => {
    const body = markdownReview(summaryFor({ governance: [] }));
    expect(body).not.toContain("### Governance");
  });

  it("truncates a body over GitHub's limit instead of letting the comment 422", () => {
    /*
      The failure this prevents is silent and total: GitHub refuses the whole comment, the
      delivery is logged as failed, and the reviewer gets nothing at all on a large merge.
    */
    const many = Array.from({ length: 4000 }, (_, index) => ({
      table: `t_${index}`,
      statements: [`ALTER TABLE \`p.d.t_${index}\` ADD COLUMN a_reasonably_long_column_name STRING`],
      requiresRecreate: false,
    }));

    const body = markdownReview(summaryFor({ migration: many }));

    expect(body.length).toBeLessThanOrEqual(65_536);
    expect(body).toContain("…truncated.");
    // The head survives, so the reader still gets the part ordered most-useful-first.
    expect(body).toContain("## Model review");
  });
});
