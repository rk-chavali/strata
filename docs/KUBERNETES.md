...
```bash
kind load docker-image strata:1.0.0 --name strata
```
...
`Always`. Run `kind load docker-image strata:1.0.0 --name <cluster>` and keep the policy at
