...
## Your editor knows the schema

Model files are YAML you type by hand, so the fastest feedback is the kind that arrives
before you save. Strata publishes a JSON Schema for its file format, generated from the same
definitions the loader validates against. Point an editor at it and you get completion on
every field, a red line under a typo, and hover documentation, without running anything:

```yaml
# yaml-language-server: $schema=https://strata-docs.chavali-r.workers.dev/schema/strata.schema.json
id: tbl_fct_order
kind: table
```

That one comment is all the VS Code YAML extension needs. To apply it to every model file at
once instead, map it by glob in your editor settings rather than repeating the line.

The schema is generated, never hand-written, and a test fails if the committed copy and the
metamodel disagree, so it cannot quietly drift into describing a format that no longer
exists. Regenerate it with `pnpm --filter @strata/metamodel schema`.

It deliberately permits fields it does not define, because the loader does: a file written by
a newer Strata keeps its unfamiliar keys through a read and a write here, and an editor
should not mark a correct file as broken just because the tool is a version behind.
`strata check` still reports those keys as warnings, which is the right severity for
something that is either a typo or the future.
