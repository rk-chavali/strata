...
export const WorkspaceConfigSchema = z.object({
  /**
   * Format version of the repository, not of the tool that wrote it.
   *
   * Any positive integer parses. `z.literal(CONFIG_VERSION)` was the obvious spelling and
   * the wrong one: it made shipping a version 2 impossible, because every Strata already
   * installed would reject the new file with a zod union error rather than a sentence, and
   * a format that cannot be versioned up is a format frozen at version 1 forever.
   * `assertSupportedVersion` is where the refusal belongs, so it can say which side is old.
   */
...
/**
 * A workspace written by a Strata newer than this one.
 *
 * Separate from a plain parse failure because the remedy is different and the user cannot
 * work it out from the file: nothing is wrong with their repository, their tool is behind.
 */
export class UnsupportedVersionError extends Error {
  constructor(
    ...
    super(
      `${CONFIG_FILENAME} is version ${found}, and this Strata understands up to version ` +
        `${supported}. Upgrade Strata to open this workspace. Opening it with this version ` +
        "would drop the parts of the format it does not know about.",
    );
...
/**
 * Refuse a workspace from the future, before anything reads it.
 *
 * Older versions load without complaint. There is nothing to migrate yet, and the field
 * preservation in `parseObject` means a version 1 repository opened by a later Strata keeps
 * whatever that version adds. When a breaking change does arrive, this is the seam it goes
 * through: read `found`, transform, and carry on.
 */
export function assertSupportedVersion(version: number): void {
  if (version > CONFIG_VERSION) throw new UnsupportedVersionError(version, CONFIG_VERSION);
}
