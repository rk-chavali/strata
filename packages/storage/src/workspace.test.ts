...
describe("resource limits", () => {
  /**
   * The loader reads whatever the repository contains, and the repository takes pull requests.
   *
   * That makes every file here untrusted input at a trust boundary, which is what these limits
   * are for: a single enormous YAML file, or tens of thousands of small ones, is a denial of
   * service against the instance that loads them and takes no privilege to attempt.
   *
   * Both cases assert the *diagnostic*, not just the absence of a crash. Refusing silently would
   * produce a workspace that looks empty, and "your model vanished" is a worse bug report than
   * "your model is too large".
   */

  it("refuses one oversized file, and keeps loading the rest", async () => {
    await seedWorkspace();
    // 9MB of valid YAML: over the 8MB ceiling, and not a parse failure.
    await write("anywhere/huge.yaml", `id: tbl_huge\nkind: table\nname: huge\nmodel: core\ndescription: "${"x".repeat(9 * 1024 * 1024)}"\n`);

    const ws = await loadWorkspace(root);
    const finding = ws.diagnostics.find((d) => d.code === "workspace/fileTooLarge");

    expect(finding?.severity).toBe("error");
    expect(finding?.file).toBe("anywhere/huge.yaml");
    expect(finding?.message).toContain("8MB limit");
    // One bad file does not cost you the workspace.
    expect(ws.graph.all()).toHaveLength(7);
  });

  it("refuses a workspace with too many files outright, rather than loading part of it", async () => {
    /*
      Wholesale, not truncated. A workspace that loaded most of its objects would validate clean,
      generate incomplete DDL, and report a model that does not exist. Being told it is too large
      is a far better failure than being quietly given the wrong answer.

      The ceiling is lowered rather than writing twenty thousand files, which would add minutes to
      every run to assert one comparison.
    */
    await seedWorkspace();

    const ws = await loadWorkspace(root, { maxFiles: 1 });
    const finding = ws.diagnostics.find((d) => d.code === "workspace/tooManyFiles");

    expect(finding?.severity).toBe("error");
    expect(finding?.message).toContain("above the limit of 1");
    // Nothing partial. An empty graph plus an error beats a plausible-looking wrong answer.
    expect(ws.graph.all()).toEqual([]);
  });

  it("loads a normal workspace unchanged, so the ceilings cost nothing in practice", async () => {
    await seedWorkspace();
    const ws = await loadWorkspace(root);
