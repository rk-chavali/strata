...
/**
 * How many model files the loader will read.
 *
 * Sized to be far above any real model and far below a denial of service. The largest warehouse
 * anyone has modelled here is in the low hundreds of objects, and `single-file-per-model` and
 * `flat` both put many objects in one file, so a genuine workspace does not approach this.
 */
const MAX_FILES = 20_000;

/**
 * How large one model file may be.
 *
 * `apps/server/src/files.ts` already caps the *viewer* at 2MB. This is the loader, which is the
 * path that actually matters: the viewer refusing to display a file is a nuisance, the loader
 * parsing a 500MB one is the process.
 */
...
/**
 * Overridable ceilings.
 *
 * Exists so the guards can be tested. The alternative was a test that writes twenty thousand
 * files to assert a comparison, which would add minutes to every run, and the alternative to
 * *that* was leaving the file-count guard unexercised, which is how a guard quietly stops
 * guarding. Callers in the product pass nothing and get the defaults.
 */
...
export async function loadWorkspace(
  ...
  /*
    A ceiling on how much of the repository this will read.

    The loader globs and parses every YAML file it finds, with no limit, and the content is
    attacker-controlled the moment a workspace takes pull requests: a junior analyst opening one
    is inside the threat model. A single 500MB YAML file, or fifty thousand small ones, is a
    denial of service against the instance that loads them, and it takes no privilege to try.

    Refused wholesale rather than truncated silently. A workspace that loaded three thousand of
    its five thousand objects would validate clean, generate incomplete DDL, and report a model
    that does not exist, which is a far worse failure than being told the workspace is too large.
    A diagnostic and no objects is the honest outcome.
  */
  if (discovered.length > maxFiles) {
    diagnostics.push({
      severity: "error",
      code: "workspace/tooManyFiles",
      message:
        `${discovered.length} model files found, above the limit of ${maxFiles}. ` +
        "Narrow `roots` or add to `ignore` in strata.config.yaml so the loader only reads model files.",
    });
    return {
      root,
      config,
      graph: ObjectGraph.from([]),
      diagnostics,
      filesByPath,
      pathById,
    };
  ...
  for (const file of discovered) {
    ...
    if (info.size > maxFileBytes) {
      diagnostics.push({
        severity: "error",
        code: "workspace/fileTooLarge",
        message:
          `${Math.round(info.size / 1024 / 1024)}MB, above the ${maxFileBytes / 1024 / 1024}MB limit for one model file. ` +
          "A model file this size is almost certainly not a model file.",
        file,
      });
      continue;
    }
    ...
    for (const doc of parsed.documents) {
      ...
      if (result.unrecognised) {
        const one = result.unrecognised.length === 1;
        diagnostics.push({
          severity: "warning",
          code: "object/unknownField",
          message:
            `unrecognised ${one ? "field" : "fields"} ` +
            `${result.unrecognised.map((f) => `\`${f}\``).join(", ")}: ` +
            `not part of the schema for this kind, so nothing reads ${one ? "it" : "them"}. ` +
            `${one ? "It is" : "They are"} kept as written and saved back unchanged, ` +
            "so a file from a newer Strata survives a round trip here. " +
            `If ${one ? "it was" : "they were"} meant to be ${one ? "a real field" : "real fields"}, ` +
            "check the spelling against the docs.",
          file,
          path: `line ${doc.line}`,
        });
      }
