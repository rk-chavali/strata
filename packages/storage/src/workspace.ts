    ...
    for (const doc of parsed.documents) {
      ...
      if (result.discarded) {
        diagnostics.push({
          ...
          message:
            `ignored ${result.discarded.length === 1 ? "field" : "fields"} ` +
            `${result.discarded.map((f) => `\`${f}\``).join(", ")}: ` +
            `not part of the schema for this kind, so nothing reads ` +
            `${result.discarded.length === 1 ? "it" : "them"}. ` +
            "Check the spelling against the docs, or delete the key.",
          file,
          path: `line ${doc.line}`,
        });
      }
