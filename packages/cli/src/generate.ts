...
export interface GenerateArgs {
  ...
  dataformOut?: string;
  /**
   * Emit an Apache Ossie semantic model.
   *
   * Separate from DDL for the same reason Dataform is: it lands somewhere else
   * and is read by something else. Strata models structure; a semantic layer
   * models measurement. This hands over the part Ossie cannot state for itself,
   * namely grain and join cardinality, both of which Strata already declares.
   */
  semantic: boolean;
  /** Where the semantic workspace lives, relative to the repo root. */
  semanticOut?: string;
}
...
export async function commandGenerate(args: GenerateArgs): Promise<ExitCode> {
  ...
  /**
   * The semantic model, written into its own folder.
   *
   * Warnings here are not cosmetic. Each one names a place where the semantic
   * engine will answer differently than the Strata model implies, which is a
   * refusal the modeller would otherwise meet with no way to explain it.
   */
  const semanticWarnings: OssieWarning[] = [];
  if (args.semantic) {
    for (const model of physical) {
      const result = generateOssie(workspace.graph, model.name, {
        out: args.semanticOut ?? "semantic",
      });
      generated.push(...result.files);
      semanticWarnings.push(...result.warnings);
    }
  }
  ...
  const all: { path: string; contents: string; dataform?: boolean; semantic?: boolean }[] = [
    ...generated.map((file) => ({ ...file, semantic: file.kind === "semantic" })),
  ...
  for (const file of all) {
    ...
    // CODEOWNERS and the semantic workspace carry paths that are already
    // relative to the repo root; DDL is written under the folder the caller
    // nominated, and Dataform under its own project root.
    const relative =
      file.path === "CODEOWNERS" || file.semantic
        ? file.path
  ...
  if (semanticWarnings.length > 0) {
    info("");
    heading(`${semanticWarnings.length} thing(s) the semantic engine will read differently`);
