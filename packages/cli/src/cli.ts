...
COMMANDS
  ...
  generate semantic Emit an Apache Ossie semantic model for a semantic layer
...
async function dispatch(command: string, args: readonly string[]): Promise<ExitCode> {
  switch (command) {
    ...
    case "generate": {
      ...
      if (what !== "ddl" && what !== "dataform" && what !== "semantic" && what !== "all") {
        failure(`unknown generate target \`${what}\` (expected \`ddl\`, \`dataform\`, \`semantic\` or \`all\`)`);
        return EXIT.failure;
      }
      ...
      return commandGenerate({
        ...
        semantic: what === "semantic" || what === "all",
        ...(values["semantic-out"] ? { semanticOut: String(values["semantic-out"]) } : {}),
