...
#
#   docker run -e STRATA_WORKSPACE=examples/quickstart ...
#
...
# Both state directories are created and chowned here, and the reason is the whole first-run
# experience.
#
# The container runs as `strata` (uid 10001), but neither `/workspace` nor the data directory
# existed in the image, so Docker created them root-owned: an implicit `VOLUME` target is made
# by the daemon, and a named volume mounted at a path absent from the image is initialised
# root:root. The server booted, served the UI, printed "open the UI to create the administrator
# account", and then answered that request with a 500 and `EACCES`. Every documented way to run
# this failed at the first thing anyone does, and the cause was only ever visible in the
# container log.
#
# Creating them in the image fixes all of it at once, because Docker seeds a fresh named volume
# from the image's content *and ownership* at that path. `STRATA_DATA_DIR` is pinned to `/data`
# for the same reason: the previous default was relative to the working directory, which put
# accounts inside `/app` where this user also cannot write.
#
# A bind mount still takes its ownership from the host, so a Linux operator mounting a directory
# owned by another uid must chown it or pass `--user`. That is documented rather than worked
# around, because guessing at host ownership from inside a container is how you end up chowning
# somebody's repository.
RUN mkdir -p /workspace /data && chown strata:strata /workspace /data

# Mount the model repo at /workspace. /data holds accounts, the session secret, encrypted
# credentials and the delivery log, and must be persisted: losing it costs accounts, not models.
